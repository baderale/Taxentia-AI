import type { Authority } from "@shared/schema";
import { storage } from "../storage";

class MockRetrievalService {
  async retrieveAuthorities(query: string, k: number = 4): Promise<Authority[]> {
    ...
    const scoredAuthorities = allAuthorities.map(authority => {
      ...
      keywords.forEach(keyword => {
        ...
        if (authority.citation.toLowerCase().includes(keyword)) score += 10;
        if (authority.title.toLowerCase().includes(keyword)) score += 5;
        if (authority.content.toLowerCase().includes(keyword)) score += 1;
        
        // Special scoring for specific terms
        if (keyword === 'startup' && authText.includes('195')) score += 20;
        if (keyword === 'home' && authText.includes('280a')) score += 20;
        if (keyword === 'office' && authText.includes('280a')) score += 15;
        if (keyword === 'qbi' && authText.includes('199a')) score += 20;
        if (keyword === 'qualified' && authText.includes('199a')) score += 15;
        if (keyword === 'business' && authText.includes('199a')) score += 10;
        if (keyword === 'deduction' && (authText.includes('195') || authText.includes('280a') || authText.includes('199a'))) score += 8;
        if (keyword === 'election' && authText.includes('195')) score += 15;
        if (keyword === 'corp' && authText.includes('2553')) score += 20;
      });
  ...
  async searchAuthorities(sourceTypes: string[] = [], searchTerm: string = ""): Promise<Authority[]> {
    ...
    return authorities.filter(auth => 
      auth.citation.toLowerCase().includes(term) ||
